...
SELECTED_FEATURES_INPUT = Path(
    "data/processed/"
    "feature_selection/"
    "heuristic_selected_features.json"
)
...
VALIDATION_METRICS_OUTPUT = (
    OUTPUT_ROOT
    / "heuristic_validation_metrics.json"
)

MANIFEST_OUTPUT = (
    OUTPUT_ROOT
    / "heuristic_training_manifest.txt"
)
...
def _read_feature_selection() -> dict:
    ...
    if not SELECTED_FEATURES_INPUT.exists():
        ...
        raise FileNotFoundError(
            (
                "Phase 17 selected-feature file "
                "does not exist: "
...
def _validate_phase17_hashes(
    ...
    """
    Prevent accidentally training RF-H using
    split files that differ from those used
    ...
    if (
        ...
        raise ValueError(
            (
                "Training split has changed "
                "since Phase 17 feature selection."
            )
        )
...
def _validate_domain_labels(
    ...
    """
    One registrable domain should not carry
    conflicting binary labels within training.
    """
...
def _choose_cv_splits(
    ...
    """
    Determine a defensible number of grouped
    cross-validation folds.

    We require at least three unique domains
...
def main():
    ...
    manifest_lines = [
        "PHASE 18 RF-H TRAINING MANIFEST",
        "=" * 72,
    ...
    print(
        "Top heuristic feature importance:"
    )
